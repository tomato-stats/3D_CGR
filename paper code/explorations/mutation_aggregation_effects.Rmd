...
cgr_output <- 
  lapply(
    fracs, 
    function(x) 
      cgr_distance3(all_mut_cg, frac = x) |> melt() |> 
      filter(Var1 == "Parent" & Var2 != "Parent") |> 
      mutate(frac = x)
  ) |> 
  (\(x) do.call("rbind", x) )() |> 
...
cgr_output <- 
  lapply(
    fracs, 
    function(x) 
      cgr_distance4(all_mut_cg, frac = x) |> melt() |> 
      filter(Var1 == "Parent" & Var2 != "Parent") |> 
      mutate(frac = x)
  ) |> 
  (\(x) do.call("rbind", x) )() |> 
...
test <- str_split(all_mutations, "")
names(test) <- names(all_mutations)
kmer::kdistance(test)

kmer_output <-
  kmer::kdistance(test) |> as.matrix() |> melt() |> 
  mutate(
    `Mutation count` = as.numeric(gsub("(Substitutions|Deletions) = ", "", Var2)),
    `Mutation percent` = `Mutation count`/nchar(parent_dna),
    `Mutation type` = gsub(" = [0-9]*", "", Var2)
...
bind_rows(
  full_join(
    ...
    by = c("Var1", "Mutation count", "Mutation percent"), 
    suffix=c(".S", ".D")
  ) |> mutate(method = "Clustal Omega, p-distance"),
  
  full_join(
    kmer_substitutions, kmer_deletions, 
    by = c("Var1", "Mutation count", "Mutation percent"), 
    suffix=c(".S", ".D")
  ) |> mutate(method = "kmer distance"),
  
  full_join(
    sub_vol, del_vol, 
    by = c("Mutation count", "Mutation percent", "bandwidth_value"), 
    suffix = c(".S", ".D")
  ) |> mutate(method = "volume intersection"),
  
  full_join(
    cgr_substitutions, cgr_deletions, 
    by = c("Var1", "Mutation count", "Mutation percent", "frac"), 
    suffix=c(".S", ".D")
  ) |> mutate(method = "shape signature")
)
...
#   ) |> 
#     ggplot() + 
#     geom_point(aes(x=value.scaled.S, y=value.scaled.D, col = `Mutation percent`), alpha = .5) + 
#     scale_color_continuous("Mutation\npercent", low = "blue", high = "orange") + 
#     theme_minimal() +  
#     geom_abline(slope = 1, intercept = 0, lty =2) +  
#     geom_vline(xintercept = 0) + geom_hline(yintercept = 0) + 
...
ggsave("mutation_aggregation.png", width =3*4.5, height = 4.5, dpi = 300)
...
```{r cgr2_ss_n, cache = T, eval = F, dpi = 300}
...
cgr2_substitutions <- cgr2_output_list.rbind |> filter(`Mutation type` == "Substitutions") 
cgr2_deletions <- cgr2_output_list.rbind |> filter(`Mutation type` == "Deletions") 
