...
beta_cg <- lapply(beta_seq, seq_to_hypercomplex_cg)  
nadh_cg <- lapply(nadh, seq_to_hypercomplex_cg)  
sim_cg <- lapply(sim_fas, seq_to_hypercomplex_cg) 

cgr_distance(beta_cg, frac = 1) |> hm()
cgr_distance(beta_cg, frac = 1) |> bionj() |> ape::root("gallus") |> plot()
cgr_distance(beta_cg, frac = 1) |> fastme.bal() |> root("gallus") |> plot() 

cgr_distance(beta_cg, frac = 1/15) |> hm()
cgr_distance(beta_cg, frac = 1/15) |> bionj() |> ape::root("gallus") |> plot()
cgr_distance(beta_cg, frac = 1/15) |> fastme.bal() |> root("gallus") |> plot() 

cgr_distance(nadh_cg, frac = 1) |> hm()
cgr_distance(nadh_cg, frac = 1) |> bionj() |> root("Saimiri sciureus") |> plot()
cgr_distance(nadh_cg, frac = 1) |> fastme.bal() |> root("Saimiri sciureus") |> plot()

cgr_distance(nadh_cg, frac = 1/15) |> hm()
cgr_distance(nadh_cg, frac = 1/15) |> bionj() |> root("Saimiri sciureus") |> plot()
cgr_distance(nadh_cg, frac = 1/15) |> fastme.bal() |> root("Saimiri sciureus") |> plot()

cgr_distance(sim_cg) |> hm()
...
volume_intersection_tanimoto(
  ...
  print(
    plot(
      hclust(as.dist(1-.), method = "complete" ), 
