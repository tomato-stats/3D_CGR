...
kmer_dist(beta_seq) |> hm() 
kmer_dist(beta_seq) |> bionj() |> root("gallus") |> plot()
kmer_dist(beta_seq) |> upgma() |> root("gallus") |> plot()
kmer_dist(beta_seq) |> wpgma() |> root("gallus") |> plot()

kmer_dist(nadh) |> hm() 
kmer_dist(nadh) |> bionj() |> root("Saimiri sciureus") |> plot()
kmer_dist(nadh) |> upgma() |> root("Saimiri sciureus") |> plot()
kmer_dist(nadh) |> wpgma() |> root("Saimiri sciureus") |> plot()

kmer_dist(syn_fas) |> hm() 
kmer_dist(syn_fas) |> bionj() |> plot()
kmer_dist(syn_fas) |> upgma() |> plot()
kmer_dist(syn_fas) |> wpgma() |> plot()
...
cgr_distance2(beta_cg, frac = 1, normalize = T) |> hm()
cgr_distance2(beta_cg, frac = 1, normalize = T) |> bionj() |> ape::root("gallus") |> plot()

cgr_distance2(beta_cg, frac = 1/15, normalize = T) |> hm()
cgr_distance2(beta_cg, frac = 1/15, normalize = T) |> bionj() |> ape::root("gallus") |> plot()

cgr_distance2(nadh_cg, frac = 1, normalize = T) |> hm()
cgr_distance2(nadh_cg, frac = 1, normalize = T) |> bionj() |> root("Saimiri sciureus") |> plot()

cgr_distance2(nadh_cg, frac = 1/15, normalize = T) |> hm()
cgr_distance2(nadh_cg, frac = 1/15, normalize = T) |> bionj() |> root("Saimiri sciureus") |> plot()

cgr_distance2(syn_cg, frac = 1, normalize = T) |> hm()
cgr_distance2(syn_cg, frac = 1, normalize = T) |> bionj() |> plot()

cgr_distance2(syn_cg, frac = 1/15, normalize = T) |> hm()
...
align_plots1 <- function (...) {
  ...
  gl <- lapply(pl, ggplotGrob)
  bind2 <- function(x, y) gtable:::cbind_gtable(x, y, "first")
  combined1 <- Reduce(bind2, gl[-1], gl[[1]])
  combined2 <- gtable:::rbind_gtable(
    combined1[1:(nrow(combined1) / 2), ],
    combined1[((nrow(combined1) / 2) + 1) : nrow(combined1), ]
  )
  wl <- lapply(gl, "[[", "heights")
  combined2$heights <- do.call(grid::unit.pmax,  wl)
  grid::grid.newpage()
  grid::grid.draw(combined2)
}


align_plots1(
    clustal_omega(beta_seq) |> hm.vir(),
    kmer_beta |> (\(x) x / max(x))() |> hm.vir(),
...
(
  clustal_omega(beta_seq) |> bionj() |> root("gallus") |> ggtree() + geom_tiplab()+ coord_cartesian(xlim = c(0, .7)) +
  kmer_dist(beta_seq)  |> bionj() |> root("gallus") |> ggtree() + geom_tiplab()+ coord_cartesian(xlim = c(0, .35)) +
  beta_tanimoto |> (\(x) 1 - x)() |> bionj() |> ape::root("gallus") |> ggtree() + geom_tiplab() + coord_cartesian(xlim = c(0, .65)) +
  cgr_distance2(beta_cg, frac = 1/15, normalize = T) |> bionj() |> root("gallus") |> ggtree() + geom_tiplab() +  coord_cartesian(xlim = c(0, .65)) +
    
  clustal_omega(nadh) |> bionj() |> root("Saimiri sciureus") |> ggtree() + geom_tiplab() + coord_cartesian(xlim = c(0, .75)) +
  kmer_dist(nadh)  |> bionj() |> root("Saimiri sciureus") |> ggtree() + geom_tiplab()+ coord_cartesian(xlim = c(0, .18)) +
  nadh_tanimoto |> (\(x) 1 - x)() |> bionj() |> root("Saimiri sciureus") |> ggtree() + geom_tiplab() + coord_cartesian(xlim = c(0, .75)) +
  cgr_distance2(nadh_cg, frac = 1/15, normalize = T) |> bionj() |> root("Saimiri sciureus") |> ggtree() + geom_tiplab() +  coord_cartesian(xlim = c(0, .75))  +
    
  clustal_omega(syn_fas) |> bionj() |> root("Original") |> ggtree(ladderize = F) + geom_tiplab() + coord_cartesian(xlim = c(0, 1.02)) +
  kmer_dist(syn_fas) |> bionj() |> root("Original") |> ggtree() + geom_tiplab()+  coord_cartesian(xlim = c(0, .022)) + 
  syn_tanimoto |> (\(x) 1 - x)() |> bionj()|> root("Original") |> ggtree(ladderize = F) + geom_tiplab() + coord_cartesian(xlim = c(0, .2)) +
  cgr_distance2(syn_cg, frac = 1/15, normalize = T) |> bionj()|> root("Original") |> ggtree(ladderize = F) + geom_tiplab() + coord_cartesian(xlim = c(0, .9)) 
) + patchwork::plot_layout(ncol = 2, byrow = T)
...
cowplot::plot_grid(
  ...
  kmer_dist(nadh)  |> bionj() |> root("Saimiri sciureus") |> ggtree() + geom_tiplab()+ coord_cartesian(xlim = c(0, .18)) ,
  nadh_tanimoto |> (\(x) 1 - x)() |> bionj() |> root("Saimiri sciureus") |> ggtree() + geom_tiplab() + coord_cartesian(xlim = c(0, .75)),
  cgr_distance2(nadh_cg, frac = 1/15, normalize = T) |> (\(x) x / max(x))() |> bionj() |> root("Saimiri sciureus") |> ggtree() + geom_tiplab() + coord_cartesian(xlim = c(0, .75)),
  
  clustal_omega(syn_fas) |> bionj() |> root("Original") |> ggtree(ladderize = F) + geom_tiplab() + coord_cartesian(xlim = c(0, 1.02)),
  kmer_dist(syn_fas) |> bionj() |> root("Original") |> ggtree() + geom_tiplab()+  coord_cartesian(xlim = c(0, .022)), 
  syn_tanimoto |> (\(x) 1 - x)() |> bionj()|> root("Original") |> ggtree(ladderize = F) + geom_tiplab() + coord_cartesian(xlim = c(0, .2)),
  cgr_distance2(syn_cg, frac = 1/15, normalize = T) |> bionj()|> root("Original") |> ggtree(ladderize = F) + geom_tiplab() + coord_cartesian(xlim = c(0, .6)),
  align = "hv", axis = "lr", ncol = 2, nrow = 6
)
