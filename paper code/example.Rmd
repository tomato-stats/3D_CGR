...
clustal_omega(syn_fas) |> fastme.bal() |> plot()
...
cgr_distance2(syn_cg, frac = 1) |> fastme.bal() |> plot() 

cgr_distance2(syn_cg, frac = 1/15) |> hm()
cgr_distance2(syn_cg, frac = 1/15) |> bionj() |> plot()
cgr_distance2(syn_cg, frac = 1/15) |> fastme.bal() |> plot() 
...
cowplot::plot_grid(
    ...
    beta_tanimoto |> (\(x) 1 - x)()  |> (\(x) x / max(x))() |> hm.vir()+ theme(legend.position = "none"),
    cgr_distance2(beta_cg, frac = 1/15, normalize = T) |> (\(x) x / max(x))() |> hm.vir()+theme(legend.position = "right"), 
    clustal_omega(nadh) |> hm.vir()+ theme(legend.position = "none"),
    nadh_tanimoto |> (\(x) 1 - x)() |> (\(x) x / max(x))() |> hm.vir() + theme(legend.position = "none") ,
    cgr_distance2(nadh_cg, frac = 1/15, normalize = T) |> (\(x) x / max(x))() |> hm.vir()+ theme(legend.position = "right"), 
    clustal_omega(syn_fas) |> hm.vir() + theme(legend.position = "none"),
    syn_tanimoto |> (\(x) 1 - x)() |> (\(x) x / max(x))() |> hm.vir() + theme(legend.position = "none"),
    cgr_distance2(syn_fas, frac = 1/15, normalize = T)|> (\(x) x / max(x))()  |> hm.vir() + theme(legend.position = "right"), 
...
plot_grid(
  ...
  (clustal_omega(syn_fas) |> bionj() |> root("Original") |> ape::rotateConstr(c("Original")) |> ggtree(ladderize = F) |> geom_tiplab() + coord_cartesian(xlim = c(0, 1))) + 
  (cgr_distance2(syn_cg, frac = 1/15) |> bionj() |> root("Original") |> ape::rotateConstr(c("Original")) |> ggtree(ladderize = F) |> geom_tiplab() + coord_cartesian(xlim = c(0, 1))) + 
  (syn_tanimoto |> (\(x) 1 - x)() |> (\(x) x / max(x))() |> bionj() |> root("Original") |> ape::rotateConstr(c("Original")) |> ggtree(ladderize = F) + geom_tiplab() + coord_cartesian(xlim = c(0, 1))) + patchwork::plot_layout(ncol = 3)
...
ggsave("trees.png", width = 35 * 1/2.6, height = 21 * 1/3, dpi = 600)
